 ...
 * 触发后端 RAG 文档导入接口。
 *
 * 调用 POST /rag/ingest，把 docs/sample-docs 下的 Markdown
 * 读取为文档元数据。
 */
 ...
 * 获取后端当前内存知识库的文档切片结果。
 *
 * 用这个接口调试 chunk metadata、顺序和正文片段。
 */
...
export async function askRag(query: string, topK: number = 5): Promise<RagAskResponse> {
